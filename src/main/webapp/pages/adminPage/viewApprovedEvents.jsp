...
<div class="flex h-screen">
  ...
  <div class="flex-1 p-6">
    <div class="container mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 class="text-3xl font-bold mb-6">Approved Events</h2>
        ...
        <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
          ...
          <tbody>
            ...
            <c:if test="${empty events}">
              <tr>
                <td colspan="5" class="text-center py-4 text-gray-500">No approved events found.</td>
              </tr>
            </c:if>
          </tbody>
        </table>
      ...
      <c:if test="${totalPages > 1}">
        <div class="mt-6 flex justify-center items-center space-x-4">
          ...
          <c:if test="${page > 1}">
            <a class="bg-blue-600 text-white py-2 px-4 rounded" href="${pageContext.request.contextPath}/admin/viewApprovedEvents?page=1">First</a>
            <a class="bg-blue-600 text-white py-2 px-4 rounded" href="${pageContext.request.contextPath}/admin/viewApprovedEvents?page=${page - 1}">Prev</a>
          </c:if>
          <c:forEach begin="${startPage}" end="${endPage}" var="i">
            <c:choose>
              ...
              <c:otherwise>
                <a class="bg-blue-600 text-white py-2 px-4 rounded" href="${pageContext.request.contextPath}/admin/viewApprovedEvents?page=${i}">${i}</a>
              </c:otherwise>
            </c:choose>
          </c:forEach>
          <c:if test="${page < totalPages}">
            <a class="bg-blue-600 text-white py-2 px-4 rounded" href="${pageContext.request.contextPath}/admin/viewApprovedEvents?page=${page + 1}">Next</a>
            <a class="bg-blue-600 text-white py-2 px-4 rounded" href="${pageContext.request.contextPath}/admin/viewApprovedEvents?page=${totalPages}">Last</a>
          </c:if>
        </div>
      </c:if>
