...
function updateShowTimeLabel(input) {
    ...
    const showTimeIndex = labelSpan.textContent.match(/\d+/) ? labelSpan.textContent.match(/\d+/)[0] : '1'; // Mặc định là 1 nếu không match được
...
// Sửa addNewShowTime để thêm sự kiện input
    ...
    // Gắn sự kiện onchange cho các input mới
    const newInputs = newShowTime.querySelectorAll('input[type="datetime-local"]');
    newInputs.forEach(input => {
        input.addEventListener('input', () => validateDateTime(input)); // Kiểm tra validate
        input.addEventListener('change', () => updateShowTimeLabel(input)); // Gắn sự kiện update label
    });
}
...
async function submitEventForm() {
    ...
    showTimeElements.forEach((showTime) => {
        ...
        if (startDate && endDate) {
            ...
            ticketElements.forEach((ticket) => {
                ...
                const ticketColorSpan = ticket.querySelector('.ticket-details div:nth-child(5) span');
                ...
                if (eventType === 'seatedevent') {
                    ...
                    selectedRows = selectedRowsText.split(', ').map(row => row.split(' ')[1] || '');
                }
                tickets.push({
                    name: ticketName,
    ...
    if (eventType === 'seatedevent') {
        ...
        seatRows.forEach((rowInput, index) => {
            ...
            const seatNum = parseInt(seatNumbers[index]?.value.trim()) || 0;
            if (row && !isNaN(seatNum) && seatNum > 0) {
                seats.push({
                    seatRow: row,
                    status: "Available"
    ...
    const eventData = {
        customerId: 10, // Replace with actual customer ID from session
        eventName: eventName,
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.redirectUrl) {
                ...
                window.location.href = 'createNewEvent?success=true';
            }
        } else {
            alert('Failed to create event: ' + (result.message || 'Unknown error. Check console for details.'));
        }
    } catch (error) {
