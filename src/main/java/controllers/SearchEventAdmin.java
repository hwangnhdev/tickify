    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (searchKeyword != null && !searchKeyword.trim().isEmpty()) {
            if ("approved".equalsIgnoreCase(tab)) {
                List<EventAdmin> events = dao.searchApprovedEventsByName(searchKeyword, page, PAGE_SIZE);
                totalRecords = dao.getTotalSearchApprovedEventsByName(searchKeyword);
                request.setAttribute("events", events);
            } else if ("history".equalsIgnoreCase(tab)) {
                List<EventAdmin> events = dao.searchHistoryEventsByName(searchKeyword, page, PAGE_SIZE);
            ...
            if ("approved".equalsIgnoreCase(tab)) {
                List<EventAdmin> events = dao.getApprovedEvents(page, PAGE_SIZE);
                totalRecords = dao.getTotalApprovedEvents();
                request.setAttribute("events", events);
            } else if ("history".equalsIgnoreCase(tab)) {
                List<EventAdmin> events = dao.getHistoryApprovedEvents(page, PAGE_SIZE);
